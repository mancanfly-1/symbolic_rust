...
class MetadataVisitor : public llvm::InstVisitor<MetadataVisitor>
{
  ...
  public:
    MetadataVisitor(PyEmitter &emitter, llvm::Module &module, bool recursive = true)
        : emitter_(emitter), module_(module), recursive(recursive)
    {
    }
...
class PyInstVisitor : public llvm::InstVisitor<PyInstVisitor>
{
  ...
  public:
    ...
    /*
    功能：根据一个指令，生成对应的python函数表达式
    过程：1.得到该指令类型对应的返回类型，指令对应的操作数的类型
    */
    template <typename T> void genPyCallFromInstruction(bool hasReturn, const std::string &fname, T &i, kwargs_t kwargs = {})
    {
        ...
        MetadataVisitor mv{emitter_, this->module_, false};
    ...
    std::string visitConstant(const llvm::Constant *c)
    {
        ...
        } else if(auto ca = llvm::dyn_cast<llvm::ConstantStruct>(c)) {
            // added by zhangqiang on 2021-02-05
            return genPyCall("constant_struct", {nameType(ca), quote(name(ca))});
        }       
        else {
#if !defined(NDEBUG) || defined(LLVM_ENABLE_DUMP)
            // c->dump();
    ...
    {
        bool has_return = i.getType()->getTypeID() != llvm::Type::TypeID::VoidTyID;
        genPyCallFromInstruction(has_return, "extractvalue", i);
    }
...
{
    MetadataVisitor mv{*this,*module_};
    mv.visit(*module_);
    for (const auto &meta : mv.getMetadata()) {
...
void PyLLVMEmitter::emitStructType(const llvm::StructType &type)
{
    ...
    std::string type_name = type.getName().str();

    stream << quote(type_name) << ",";
